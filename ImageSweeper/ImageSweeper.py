...
class Run(object):
    intro = ("dxrp's chan Image Sweeper.                                      |\n"
             "Version 0.1a - built 22:46 21/12/16                             |\n"
             "Project home - https://github.com/dxrp/image-sweeper            |\n"
    ...
    def __init__(self):
        print("Checking github-repo for updates... ",end="")
        update = Update()
        if update.check_vers():
            print("update found! Type update to download.\n")
        else:
    ...
    def start_sweep(self, thread, _res):
        Sweep.start(self, thread, _res)
    ...
    def is_valid_action(self, action):
        if action == "?" or action == "update" or self.is_link(action):
            return True
        else:
            return False
